...
struct SampleBuffer {
    SampleBuffer(const int pixelWidth, const int pixelHeight) : pixelWidth(pixelWidth), pixelHeight(pixelHeight) {
...
inline void CopyBuffer(const SampleBuffer &oBuffer, SampleBuffer &desBuffer) {
    assert(oBuffer.pixelWidth == desBuffer.pixelWidth);
    assert(oBuffer.pixelHeight == desBuffer.pixelHeight);
    for (int i = 0; i < oBuffer.pixelWidth * oBuffer.pixelHeight; i++) {
        const Pixel &pixelIn = oBuffer.pixels[i];
        Pixel &pixelOut = desBuffer.pixels[i];
        for (int j = 0; j < int(pixelOut.size()); j++) {
            pixelOut[j] = 0.0;
            pixelOut[j].Add(pixelIn[j]);
        }
    }
}

inline void BufferToFilm(const SampleBuffer &buffer, Image3 *film, const Float factor = Float(1.0)) {
