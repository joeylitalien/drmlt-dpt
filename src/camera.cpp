...
void SamplePrimary(const Camera *camera, const Vector2 screenPos, const Float time, RaySegment &raySeg) {
    Ray &ray = raySeg.ray;
...
void SamplePrimary(const ADMatrix4x4 &sampleToCam, const ADAnimatedTransform &camToWorld, const ADVector2 screenPos, const ADFloat time, const bool isStatic, ADRay &ray) {
    ...
    ray.org = XformPoint(toWorld, ADVector3(Const<ADFloat>(0.0), Const<ADFloat>(0.0), Const<ADFloat>(0.0)));
    ray.dir = XformVector(toWorld, ray.dir);
}
