...
void Forward(std::shared_ptr<CFGBlock> &block,
    ...
    if (block->next != nullptr) {
        ...
        for (int oid = 0; oid < (int)split->outputs.size(); oid++) {
            ...
            for (const auto &expr : possibleExprs[oid]) {
...
void Emit(EmitHelper &helper,
    ...
    if (block->next != nullptr) {
        ...
        for (const auto &cond : split->conditions) {
            if (cond != nullptr) {
...
void Library::RegisterFuncDerv(const std::shared_ptr<Function> func,
    ...
    if (hessian)
        ...
        EmitGrad(func, wrt, dep, dfs, emitIspc);
...
void Library::Link() {
    ...
    std::string cmd = std::string("make -j12 -f Makefile.lib ") + libFilepath;
