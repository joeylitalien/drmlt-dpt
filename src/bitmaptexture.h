...
class BitmapTexture : public Texture<nChannels> {
    ...
    BitmapTexture(const OIIO::ustring &filename,
                  const Vector2 stScaler = Vector2(Float(1.0), Float(1.0)));
...
BitmapTexture<nChannels>::BitmapTexture(const std::string &filename, const Vector2 stScaler)
    : filename(OIIO::ustring(filename)),
      stScaler(stScaler),
      avg(ComputeAvg()),
      gamma(GetGamma()) {
    options.swrap = OIIO::TextureOpt::Wrap::WrapPeriodic;
    options.twrap = OIIO::TextureOpt::Wrap::WrapPeriodic;
}

template <int nChannels>
BitmapTexture<nChannels>::BitmapTexture(const OIIO::ustring &filename,
                                        const Vector2 stScaler)
...
TVector<Float, nChannels> BitmapTexture<nChannels>::Eval(const Vector2 st) const {
    ...
    if (!TextureSystem::s_TextureSystem->texture(filename,
        ...
        std::cerr << "Error:" << OIIO::geterror() << std::endl;
...
TVector<Float, nChannels> BitmapTexture<nChannels>::ComputeAvg() const {
    OIIO::ImageBuf img(filename);
    OIIO::ImageSpec spec = img.nativespec();
    Float gamma = Float(1.0);
    if (spec.format == OIIO::TypeDesc::UINT8) {
        gamma = Float(2.2);
    }
    OIIO::ImageBufAlgo::pow(img, img, gamma);
    OIIO::ImageBufAlgo::PixelStats stats;
    OIIO::ImageBufAlgo::computePixelStats(stats, img);
