...
static Float ShadingNormalCorrection(const Vector3& wi, const Intersection& isect, const Vector3& wo)
{
    // prevent light leak
...
template<bool adjoint>
static ADFloat ShadingNormalCorrection(const ADVector3& wi, const ADIntersection& isect, const ADVector3& wo)
{
    if (adjoint) {
        // Adjoint BSDF factor for shading normal
...
static inline bool Intersect(const Scene* scene, const Float time, const RaySegment& raySeg, ShapeInst& shapeInst, Intersection& isect)
{
    bool hit = false;
...
static inline const Light* GetHitLight(const Scene* scene, const bool hitSurface, const Shape* shape)
{
...
static void HandleHitLight(const int camDepth, const Scene* scene, const Light* light, const bool hitSurface, UniPathState& pathState, const Float time, const Vector2 screenPos, LightInst& envLightInst, std::vector<SubpathContrib>& contribs)
{
    ...
    Float directPdf;
    Float emissionPdf;
    light->Emission(scene->bSphere, ray.dir, isect.shadingNormal, time, lPrimID, emission, directPdf, emissionPdf);
    ...
    if (emission.sum()>Float(0.0)) {
        ...
        if (score>Float(0.0)) {
            ...
            const Float lensScore = camDepth>=2 ? Luminance(pathState.lensThroughput) : Float(0.0);
...
static inline void DirectLightingInit(const Scene* scene, SurfaceVertex& surfaceVertex, Float& lightPickProb, RNG& rng)
{
        ...
        std::vector<SubpathContrib>& contribs)
{
    ...
    Float directPdf;
    Float emissionPdf;
    if (!light->SampleDirect(scene->bSphere, isect.position, isect.shadingNormal, surfVertex.directLightRndParam, time, lPrimID, dirToLight, distToLight, lightContrib, cosAtLight, directPdf, emissionPdf)) {
        return;
    }
    ...
    bsdf->Evaluate(wi, isect.shadingNormal, dirToLight, shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    ...
    if (camDepth==1) {
        bool useAbsoluteParam = bsdf->Roughness(shapeInst.st, surfVertex.bsdfDiscrete)>scene->options->roughnessThreshold;
        if (useAbsoluteParam) {
            Vector3 impBsdfContrib;
            Float impCosWo, impBsdfPdf, impBsdfRevPdf;
            bsdf->EvaluateAdjoint(dirToLight, isect.shadingNormal, wi, shapeInst.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfRevPdf);
...
static bool BSDFSampling(const int camDepth, const Float roughnessThreshold, UniPathState& pathState, SurfaceVertex& surfVertex, Vector3& bsdfContrib)
{
    const ShapeInst& shapeInst = surfVertex.shapeInst;
    const BSDF* bsdf = shapeInst.obj->bsdf.get();
    const Vector3& wi = pathState.wi;
    ...
    Float cosWo;
    Float bsdfPdfRev;
    surfVertex.useAbsoluteParam = BoolToFloat(bsdf->Roughness(shapeInst.st, surfVertex.bsdfDiscrete)>roughnessThreshold);
    if (!perturb || surfVertex.useAbsoluteParam==FFALSE) {
        if (!bsdf->Sample(wi, isect.shadingNormal, shapeInst.st, surfVertex.bsdfRndParam, surfVertex.bsdfDiscrete, ray.dir, bsdfContrib, cosWo, bsdfPdf, bsdfPdfRev)) {
            return false;
        }
        ...
        bsdf->Evaluate(wi, isect.shadingNormal, ray.dir, shapeInst.st, bsdfContrib,  cosWo, bsdfPdf, bsdfPdfRev);
    ...
    if (surfVertex.useAbsoluteParam==FTRUE) {
        if (camDepth==0) {
            ...
            bsdf->EvaluateAdjoint(ray.dir, isect.shadingNormal, wi, shapeInst.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfPdfRev);
...
static bool RussianRoulette(const int depth, const Vector3& bsdfContrib, Float& rrWeight, Vector3& throughput, RNG& rng)
{
    std::uniform_real_distribution<Float> uniDist(Float(0.0), Float(1.0));
...
void GeneratePath(const Scene* scene, const Vector2i screenPosi, const int minDepth, const int maxDepth, Path& path, std::vector<SubpathContrib>& contribs, RNG& rng)
{
    ...
    Vector2 screenPos = Vector2(
            screenPosi[0]==-1 ? uniDist(rng) : ((screenPosi[0]+uniDist(rng))/Float(GetPixelWidth(camera))),
            screenPosi[1]==-1 ? uniDist(rng) : ((screenPosi[1]+uniDist(rng))/Float(GetPixelHeight(camera))));
    ...
    for (int camDepth = 0;; camDepth++) {
        ...
        bool hitSurface = Intersect(scene, time, pathState.raySeg, surfVertex.shapeInst, pathState.isect);

        const Light* light = GetHitLight(scene, hitSurface, surfVertex.shapeInst.obj);
        if (light!=nullptr) {
            if (camDepth+1>=minDepth) {
                if (scene->options->useLightCoordinateSampling && camDepth>1 && light->GetType()==LightType::AreaLight) {
                    assert(hitSurface);
                    SurfaceVertex& prevSurfVertex = path.camSurfaceVertex[path.camSurfaceVertex.size()-2];
                    ...
                    const ShapeInst& shapeInst = surfVertex.shapeInst;
                    prevSurfVertex.bsdfRndParam = shapeInst.obj->GetSampleParam(shapeInst.primID, pathState.isect.position, path.time);
                    ...
                    pathState.ssJacobian *= fabs(Dot(dirToPrev, pathState.isect.shadingNormal)*invDistSq)* (pathState.lcJacobian/surfVertex.shapeInst.obj->SamplePdf());
                }
                HandleHitLight(camDepth, scene, light, hitSurface, pathState, time, screenPos, path.envLightInst, contribs);
        ...
        if (camDepth==1) {
            ...
            const Float distSq = DistanceSquared(pathState.isect.position, pathState.raySeg.ray.org);
            if (distSq<=Float(0.0)) {
                contribs.clear();
                return;
            } else {
        ...
        if (camDepth+2>=minDepth) {
            ...
            DirectLighting(camDepth, scene, pathState, time, screenPos, directLightPickProb, true, surfVertex, contribs);
        }

        surfVertex.bsdfRndParam = Vector2(uniDist(rng), uniDist(rng));

        Vector3 bsdfContrib;
        if (!BSDFSampling(camDepth, scene->options->roughnessThreshold, pathState, surfVertex, bsdfContrib)) {
            break;
        }

        if (!RussianRoulette(camDepth, bsdfContrib, surfVertex.rrWeight, pathState.throughput, rng)) {
            break;
        }
...
static inline void EmitFromCameraInit(const Camera* camera,const Vector2i screenPosi, CameraVertex& camVertex, RNG& rng)
{
    std::uniform_real_distribution<Float> uniDist(Float(0.0), Float(1.0));
...
static void EmitFromCamera(const Float time, const Camera* camera, const CameraVertex& camVertex, RaySegment& raySeg, BidirPathState& pathState)
{
...
static void EmitFromLight(const BSphere& bSphere, const Float lightPickProb, const Float time, LightVertex& lgtVertex, Ray& ray, BidirPathState& pathState)
{
    const Light* light = lgtVertex.lightInst.light;
    Float cosLight;
    Float emissionPdf;
    Float directPdf;
    light->Emit(bSphere, lgtVertex.rndParamPos, lgtVertex.rndParamDir, time, lgtVertex.lightInst.lPrimID, ray, pathState.throughput, cosLight, emissionPdf, directPdf);
...
static inline void ConvertMIS(const int depth, const Light* light, const Ray& ray, BidirPathState& pathState)
{
        ...
        std::vector<SubpathContrib>& contribs)
{
    ...
    bsdf->EvaluateAdjoint(pathState.wi, pathState.isect.shadingNormal, dirToCamera, lgtVertex.shapeInst.st, bsdfContrib, cosToCamera, bsdfPdf, bsdfRevPdf);
    ...
    bool useAbsoluteParam = bsdf->Roughness(lgtVertex.shapeInst.st, lgtVertex.bsdfDiscrete)> scene->options->roughnessThreshold;
    Vector3 lensContrib = Vector3::Zero();
    if (useAbsoluteParam && lgtDepth>=1) {
        Vector3 bsdfContrib;
        Float cosWo, bsdfPdf, bsdfRevPdf;
        bsdf->Evaluate(dirToCamera, pathState.isect.shadingNormal, pathState.wi, lgtVertex.shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    ...
    const Float wLight = MIS(cameraPdf/screenPixelCount)* (pathState.accMISWPrev+pathState.accMISWThis*MIS(bsdfRevPdf));
...
static bool BSDFSampling(const Float roughnessThreshold, const int depth, const BidirPathState& pathState, SurfaceVertex& surfVertex, BidirPathState& nextPathState, Vector3& dir, Vector3& bsdfContrib)
{
    ...
    Float bsdfPdf;
    Float bsdfRevPdf;
    surfVertex.useAbsoluteParam = BoolToFloat(bsdf->Roughness(surfVertex.shapeInst.st, surfVertex.bsdfDiscrete)>roughnessThreshold);
    if (!perturb || surfVertex.useAbsoluteParam==FFALSE) {
        if (adjoint) {
            if (!bsdf->SampleAdjoint(pathState.wi, pathState.isect.shadingNormal, surfVertex.shapeInst.st, surfVertex.bsdfRndParam, surfVertex.bsdfDiscrete, dir, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf)) {
                return false;
            }
        } else {
            if (!bsdf->Sample(pathState.wi, pathState.isect.shadingNormal, surfVertex.shapeInst.st, surfVertex.bsdfRndParam, surfVertex.bsdfDiscrete, dir, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf)) {
                return false;
            }
        }
        if (surfVertex.useAbsoluteParam==FTRUE) {
        ...
        if (adjoint) {
            bsdf->EvaluateAdjoint(pathState.wi, pathState.isect.shadingNormal, dir, surfVertex.shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
        } else {
            bsdf->Evaluate(pathState.wi, pathState.isect.shadingNormal, dir, surfVertex.shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
        }
        if (bsdfContrib.isZero() || bsdfPdf<=Float(0.0)) {
            return false;
        }
    ...
    if (surfVertex.useAbsoluteParam==FTRUE) {
        ...
        if (adjoint) {
            ...
            if (fabs(cosWo)<Float(1e-2) || fabs(Dot(dir, pathState.isect.geomNormal))<Float(1e-2)) {
                nextPathState.lensContrib = Vector3::Zero();
            } else {
                nextPathState.lensContrib = pathState.lensContrib.cwiseProduct(bsdfContrib*bsdfPdf);
            }
        } else if (depth==1) {
            Vector3 impBsdfContrib;
            Float impCosWo, impBsdfPdf, impBsdfPdfRev;
            bsdf->EvaluateAdjoint(dir,
                    ...
                    impBsdfPdf,
                    impBsdfPdfRev);
            if (fabs(impCosWo)<Float(1e-2) || fabs(Dot(pathState.wi, pathState.isect.geomNormal))<Float(1e-2)) {
                nextPathState.lensContrib = Vector3::Zero();
            } else {
                const Float factor = ShadingNormalCorrection<true>(dir, pathState.isect, pathState.wi);
                impBsdfContrib *= factor;
    ...
    nextPathState.accMISWThis = MIS(cosWo/bsdfPdf)*(pathState.accMISWThis*MIS(bsdfRevPdf)+pathState.accMISWPrev);
    nextPathState.accMISWPrev = MIS(inverse(bsdfPdf));
    nextPathState.throughput = pathState.throughput.cwiseProduct(bsdfContrib);
    return true;
}

static void DirectLighting(const int camDepth, const Scene* scene, const Float time, const BidirPathState& pathState, const Vector2 screenPos, const Float lightPickProb, SurfaceVertex& camVertex, const bool doOcclusion, const bool bidirMIS, std::vector<SubpathContrib>& contribs)
{
    ...
    Float directPdf;
    Float emissionPdf;
    if (!light->SampleDirect(scene->bSphere, pathState.isect.position, pathState.isect.shadingNormal, camVertex.directLightRndParam, time, lPrimID, dirToLight, dist, lightContrib, cosAtLight, directPdf, emissionPdf)) {
        return;
    }
    ...
    Float bsdfPdf;
    Float bsdfRevPdf;
    bsdf->Evaluate(pathState.wi, pathState.isect.shadingNormal, dirToLight, camVertex.shapeInst.st, bsdfContrib, cosToLight, bsdfPdf, bsdfRevPdf);
    if (bsdfContrib.isZero()) {
        return;
    }
    ...
    if (camDepth==1) {
        bool useAbsoluteParam = bsdf->Roughness(camVertex.shapeInst.st, camVertex.bsdfDiscrete)> scene->options->roughnessThreshold;
        if (useAbsoluteParam) {
            const Intersection& isect = pathState.isect;
            Vector3 impBsdfContrib;
            Float impCosWo, impBsdfPdf, impBsdfRevPdf;
            bsdf->EvaluateAdjoint(dirToLight, isect.shadingNormal, pathState.wi, camVertex.shapeInst.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfRevPdf);
            const Float factor = ShadingNormalCorrection<true>(dirToLight, pathState.isect, pathState.wi);
            impBsdfContrib *= factor;
    ...
    if (bidirMIS) {
        ...
        Float wCamera = MIS(emissionPdf*cosToLight/(directPdf*cosAtLight))* (pathState.accMISWPrev+pathState.accMISWThis*MIS(bsdfRevPdf));
...
static void HandleHitLight(const int camDepth, const Scene* scene, const Light* light, const bool hitSurface, const Ray& ray, const Float time, const Vector2 screenPos, const BidirPathState& pathState, const bool bidirMIS, LightInst& envLightInst, std::vector<SubpathContrib>& contribs)
{
    LightPrimID lPrimID;
    Vector3 emission;
    Float directPdf;
    Float emissionPdf;
    light->Emission(scene->bSphere, ray.dir, pathState.isect.shadingNormal, time, lPrimID, emission, directPdf, emissionPdf);
    if (emission.sum()>Float(0.0)) {
        ...
        if (camDepth>0) {
            ...
            if (bidirMIS) {
                ...
                Float wCamera = MIS(directPdf)*pathState.accMISWPrev+ MIS(emissionPdf)*pathState.accMISWThis;
        ...
        std::vector<SubpathContrib>& contribs)
{
    ...
    camBSDF->Evaluate(camPathState.wi, camPathState.isect.shadingNormal, dirToLight, camVertex.shapeInst.st, camBsdfFactor, cosCamera, camBsdfPdf, camBsdfRevPdf);

    if (camBsdfFactor.isZero()) {
        return;
    }

    Float camFactor = ShadingNormalCorrection<false>(camPathState.wi, camPathState.isect, dirToLight);
    ...
    lgtBSDF->EvaluateAdjoint(lgtPathState.wi, lgtPathState.isect.shadingNormal, -dirToLight, lgtVertex.shapeInst.st, lgtBsdfFactor, cosLight, lgtBsdfPdf, lgtBsdfRevPdf);

    if (lgtBsdfFactor.isZero()) {
        return;
    }

    Float lgtFactor = ShadingNormalCorrection<true>(lgtPathState.wi, lgtPathState.isect, -dirToLight);
    ...
    if (camDepth==0) {
        bool camUseAbsoluteParam = camBSDF->Roughness(camVertex.shapeInst.st, camVertex.bsdfDiscrete)> scene->options->roughnessThreshold;
        bool lgtUseAbsoluteParam = lgtBSDF->Roughness(lgtVertex.shapeInst.st, lgtVertex.bsdfDiscrete)> scene->options->roughnessThreshold;
        ...
        if (camUseAbsoluteParam) {
            ...
            Vector3 impBsdfContrib;
            Float impCosWo, impBsdfPdf, impBsdfRevPdf;
            camBSDF->EvaluateAdjoint(dirToLight, isect.shadingNormal, camPathState.wi, camVertex.shapeInst.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfRevPdf);
    ...
    const Float wLight = MIS(camBsdfDirPdfA)* (lgtPathState.accMISWPrev+lgtPathState.accMISWThis*MIS(lgtBsdfRevPdf));
    const Float wCamera = MIS(lgtBsdfDirPdfA)*(camPathState.accMISWPrev+ camPathState.accMISWThis*MIS(camBsdfRevPdf));
...
void GeneratePathBidir(const Scene* scene, const Vector2i screenPosi, const int minDepth, const int maxDepth, Path& path, std::vector<SubpathContrib>& contribs, RNG& rng)
{
    ...
    RaySegment raySeg;
    EmitFromLight(scene->bSphere, lightPickProb, path.time, path.lgtVertex, raySeg.ray, lightPathStates[0]);
    ...
    for (int lgtDepth = 0;; lgtDepth++) {
        ...
        bool hitSurface = Intersect(scene, path.time, raySeg, surfVertex.shapeInst, lightPathStates[lgtDepth].isect);
        ...
        if (lgtDepth+2>=minDepth) {
            ConnectToCamera(lgtDepth, scene, camera, path.time, lightPathStates[lgtDepth], path.lgtSurfaceVertex[lgtDepth], prevLensContrib, raySeg.ray.org, contribs);
        }
        ...
        surfVertex.bsdfRndParam = Vector2(uniDist(rng), uniDist(rng));
        Vector3 bsdfContrib;
        if (!BSDFSampling<true>(scene->options->roughnessThreshold, lgtDepth, lightPathStates[lgtDepth], surfVertex, lightPathStates[lgtDepth+1], raySeg.ray.dir, bsdfContrib)) {
            lightPathStates.pop_back();
            break;
        }

        if (!RussianRoulette(lgtDepth, bsdfContrib, surfVertex.rrWeight, lightPathStates[lgtDepth+1].throughput, rng)) {
            lightPathStates.pop_back();
            break;
        }
    ...
    for (int camDepth = 0;; camDepth++) {
        path.camSurfaceVertex.push_back(SurfaceVertex());
        SurfaceVertex& surfVertex = path.camSurfaceVertex.back();
        bool hitSurface = Intersect(scene, path.time, raySeg, surfVertex.shapeInst, camPathState.isect);
        ...
        if (camDepth+1>=minDepth) {
            ...
            if (light!=nullptr) {
                if (scene->options->useLightCoordinateSampling && camDepth>1 && light->GetType()==LightType::AreaLight) {
                    ...
                    // something like Manifold exploration (and it's derivatives)
                    const ShapeInst& shapeInst = surfVertex.shapeInst;
                    prevSurfVertex.bsdfRndParam = shapeInst.obj->GetSampleParam(shapeInst.primID, camPathState.isect.position, path.time);
                    // Correct the jacobian since we've changed the sampling method
                    ...
                    const Float invDist = sqrt(invDistSq);
                    dirToPrev *= invDist;
                    camPathState.ssJacobian *= fabs(Dot(dirToPrev, camPathState.isect.shadingNormal)*invDistSq)* (camPathState.lcJacobian*surfVertex.shapeInst.obj->SamplePdf());
                }
                HandleHitLight(camDepth, scene, light, hitSurface, raySeg.ray, path.time, path.camVertex.screenPos, camPathState, true, path.envLightInst, contribs);
        ...
        if (camDepth+2>=minDepth) {
            Float directLightPickProb = Float(1.0);
            DirectLightingInit(scene, surfVertex, directLightPickProb, rng);
            DirectLighting(camDepth, scene, path.time, camPathState, path.camVertex.screenPos, directLightPickProb, surfVertex, true, true, contribs);
        }

        int maxLgtDepth = maxDepth==-1 ? ((int) lightPathStates.size()-1) : std::min((maxDepth-camDepth-3), ((int) lightPathStates.size()-1));
        for (int lgtDepth = 0; lgtDepth<=maxLgtDepth; lgtDepth++) {
            if (camDepth+lgtDepth+3>=minDepth) {
                ConnectVertex(camDepth, lgtDepth, scene, path.time, lightPathStates[lgtDepth], path.lgtSurfaceVertex[lgtDepth], camPathState, surfVertex, path.camVertex.screenPos, true, contribs);
            }
        }

        surfVertex.bsdfRndParam = Vector2(uniDist(rng), uniDist(rng));
        Vector3 bsdfContrib;
        if (!BSDFSampling<false>(scene->options->roughnessThreshold, camDepth, camPathState, surfVertex, camPathState, raySeg.ray.dir, bsdfContrib)) {
            break;
        }

        if (!RussianRoulette(camDepth, bsdfContrib, surfVertex.rrWeight, camPathState.throughput, rng)) {
            break;
        }
...
void GenerateSubpath(const Scene* scene, const Vector2i screenPosi, const int camLength, const int lgtLength, const bool bidirMIS, Path& path, std::vector<SubpathContrib>& contribs, RNG& rng)
{
    ...
    if (lgtLength>1) {
        Float lightPickProb = Float(1.0);
        EmitFromLightInit(scene, path.lgtVertex, lightPickProb, rng);
        EmitFromLight(scene->bSphere, lightPickProb, path.time, path.lgtVertex, raySeg.ray, lightPathState);
        raySeg.minT = c_IsectEpsilon;
        raySeg.maxT = std::numeric_limits<Float>::infinity();
        Vector3 prevLensContrib = Vector3::Zero();
        for (int lgtDepth = 0;; lgtDepth++) {
            path.lgtSurfaceVertex.push_back(SurfaceVertex());
            SurfaceVertex& surfVertex = path.lgtSurfaceVertex.back();
            bool hitSurface = Intersect(scene, path.time, raySeg, surfVertex.shapeInst, lightPathState.isect);
            ...
            if (lgtDepth+2==lgtLength) {
                if (camLength==1) {
                    ConnectToCamera(lgtDepth, scene, camera, path.time, lightPathState, path.lgtSurfaceVertex[lgtDepth], prevLensContrib, raySeg.ray.org, contribs);
                    return;
                }
                break;
            }
            ...
            surfVertex.bsdfRndParam = Vector2(uniDist(rng), uniDist(rng));
            Vector3 bsdfContrib;
            if (!BSDFSampling<true>(scene->options->roughnessThreshold, lgtDepth, lightPathState, surfVertex, lightPathState, raySeg.ray.dir, bsdfContrib)) {
                return;
            }
    ...
    for (int camDepth = 0;; camDepth++) {
        ...
        if (camDepth+2>=camLength && lgtLength==0) {
            ...
            if (light!=nullptr) {
                ...
                HandleHitLight(camDepth, scene, light, hitSurface, raySeg.ray, path.time, path.camVertex.screenPos, camPathState, bidirMIS, path.envLightInst, contribs);
            }
            return;
        }
        ...
        if (camDepth+2==camLength) {
            ...
            if (lgtLength==1) {
                Float directLightPickProb = Float(1.0);
                DirectLightingInit(scene, surfVertex, directLightPickProb, rng);
                DirectLighting(camDepth,scene,path.time,camPathState,path.camVertex.screenPos,directLightPickProb,surfVertex,true,bidirMIS,contribs);
            } else {
                assert(lgtLength>=2 && int(path.lgtSurfaceVertex.size())==lgtLength-1);
                ConnectVertex(camDepth,lgtLength-2,scene,path.time,lightPathState,path.lgtSurfaceVertex.back(),camPathState,surfVertex,path.camVertex.screenPos,true,contribs);
            }
            return;
        }
...
bool LightCoordinateSampling(const int camDepth,const Scene* scene,const Float time,const SurfaceVertex& curSurfVertex,const SurfaceVertex& nextSurfVertex, const bool doOcclusion,UniPathState& pathState,Vector3& bsdfContrib)
{
    ...
    nextShapeInst.obj->Sample(curSurfVertex.bsdfRndParam,time,nextSurfVertex.shapeInst.primID,nextPosition,nextNormal,&shapePdf);
    ...
    Float cosWo, bsdfPdf, bsdfRevPdf;
    bsdf->Evaluate(wi, isect.shadingNormal, ray.dir, shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    if (bsdfContrib.isZero()) {
        return false;
    }
    ...
    pathState.ssJacobian *= fabs(Dot(ray.dir, nextNormal)*inverse(distToLightSq))*bsdfPdf/shapePdf;
    pathState.lastBsdfPdf = bsdfPdf;
    ray.org = isect.position;

    if (camDepth==1) {
        bool useAbsoluteParam = bsdf->Roughness(shapeInst.st, curSurfVertex.bsdfDiscrete)> scene->options->roughnessThreshold;
        if (useAbsoluteParam) {
            Vector3 impBsdfContrib;
            Float impCosWo, impBsdfPdf, impBsdfRevPdf;
            bsdf->EvaluateAdjoint(ray.dir,isect.shadingNormal,wi, shapeInst.st,impBsdfContrib, impCosWo,impBsdfPdf,impBsdfRevPdf);
            const Float factor = ShadingNormalCorrection<true>(ray.dir, isect, wi);
            impBsdfContrib *= factor;
            pathState.lensThroughput = pathState.lensThroughput.cwiseProduct(impBsdfContrib);
        } else {
...
void PerturbPath(const Scene* scene, const Vector& offset, Path& path, std::vector<SubpathContrib>& contribs, RNG& rng)
{
    ...
    int offsetId = 0;
    if (path.isMoving) {
        Perturb(path.time, offset, offsetId);
    } else {
    ...
    for (int camDepth = 0; camDepth<(int) path.camSurfaceVertex.size(); camDepth++) {
        ...
        if (useLightCoordinatesPerturb) {
            ...
            hitSurface = shapeInst.obj->Intersect(shapeInst.primID, path.time, pathState.raySeg, pathState.isect, shapeInst.st);
            if (!hitSurface) {
                return;
            }
        } else {
            hitSurface = Intersect(scene, path.time, pathState.raySeg, surfVertex.shapeInst, pathState.isect);
        }
        pathState.wi = -pathState.raySeg.ray.dir;
        if (camDepth==(int) path.camSurfaceVertex.size()-1) {
            if (path.lgtDepth==0) {
                const Light* light = GetHitLight(scene, hitSurface, surfVertex.shapeInst.obj);
                if (light!=nullptr) {
                    HandleHitLight(camDepth,scene, light,hitSurface,pathState,path.time,path.camVertex.screenPos,path.envLightInst,contribs);
                }
                ...
                if (camDepth==1) {
                    path.lensVertexPos = pathState.isect.position;
                    pathState.lensThroughput *= inverse(DistanceSquared(pathState.isect.position, pathState.raySeg.ray.org));
                }
                ...
                DirectLighting(camDepth, scene, pathState,path.time,path.camVertex.screenPos, directLightPickProb, true, surfVertex, contribs);
            }
            return;
        }
        ...
        Vector3 bsdfContrib;
        if (useLightCoordinatesPerturb) {
            if (!LightCoordinateSampling(camDepth, scene, path.time, surfVertex, path.camSurfaceVertex.back(), true, pathState, bsdfContrib)) {
                return;
            }
        } else {
            if (!BSDFSampling<true>(camDepth, scene->options->roughnessThreshold, pathState, surfVertex, bsdfContrib)) {
                return;
            }
        }
...
bool LightCoordinateSampling(const int camDepth, const Scene* scene, const Float time, const SurfaceVertex& curSurfVertex, const SurfaceVertex& nextSurfVertex, const bool doOcclusion, BidirPathState& pathState, Vector3& dir, Vector3& bsdfContrib)
{
    const ShapeInst& shapeInst = curSurfVertex.shapeInst;
    const ShapeInst& nextShapeInst = nextSurfVertex.shapeInst;
    const BSDF* bsdf = shapeInst.obj->bsdf.get();
    ...
    Float cosWo, bsdfPdf, bsdfRevPdf;
    bsdf->Evaluate(wi, isect.shadingNormal, dir, shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    if (bsdfContrib.isZero()) {
        return false;
    }
    ...
    pathState.accMISWThis = MIS(cosWo/bsdfPdf)*(pathState.accMISWThis*MIS(bsdfRevPdf)+pathState.accMISWPrev);
    ...
    if (camDepth==1) {
        ...
        if (useAbsoluteParam) {
            Vector3 impBsdfContrib;
            Float impCosWo, impBsdfPdf, impBsdfRevPdf; bsdf->EvaluateAdjoint(dir, isect.shadingNormal, wi, shapeInst.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfRevPdf);
...
void PerturbPathBidir(const Scene* scene, const Vector& offset, Path& path, std::vector<SubpathContrib>& contribs, RNG& rng)
{
    ...
    if (path.lgtDepth>1) {
        ...
        for (int lgtDepth = 0; lgtDepth<(int) path.lgtSurfaceVertex.size(); lgtDepth++) {
            ...
            Perturb(surfVertex.bsdfRndParam[1], offset, offsetId);
            Vector3 bsdfContrib;
            if (!BSDFSampling<true, true>(scene->options->roughnessThreshold, lgtDepth, lightPathState, surfVertex, lightPathState, raySeg.ray.dir, bsdfContrib)) {
                return;
            }
    ...
    for (int camDepth = 0; camDepth<(int) path.camSurfaceVertex.size(); camDepth++) {
        ...
        if (camDepth==(int) path.camSurfaceVertex.size()-1) {
            if (path.lgtDepth==1) {
                ...
                const Float directLightPickProb = PickLightProb(scene, surfVertex.directLightInst.light);
                Perturb(surfVertex.directLightRndParam[0], offset, offsetId);
                Perturb(surfVertex.directLightRndParam[1], offset, offsetId);
                DirectLighting(camDepth, scene, path.time, camPathState, path.camVertex.screenPos, directLightPickProb, surfVertex, true, true, contribs);
            } else {  // path.lgtDepth > 1
                ConnectVertex(camDepth, path.lgtSurfaceVertex.size()-1, scene, path.time, lightPathState, path.lgtSurfaceVertex.back(), camPathState, surfVertex, path.camVertex.screenPos, true, contribs);
            }
            return;
        }
        ...
        Vector3 bsdfContrib;
        if (useLightCoordinatesPerturb) {
            if (!LightCoordinateSampling(camDepth, scene, path.time, surfVertex, path.camSurfaceVertex.back(), true, camPathState, raySeg.ray.dir, bsdfContrib)) {
                return;
            }
        } else {
            if (!BSDFSampling<false, true>(scene->options->roughnessThreshold, camDepth, camPathState, surfVertex, camPathState, raySeg.ray.dir, bsdfContrib)) {
                return;
            }
        }
...
/*
 * pertub the first vertex of a camera path.
 * This function is called by PerturbLens that pertub a whole camera path
 * */
bool PerturbLensFirstVertex(const Scene* scene, const Float roughnessThreshold, const Vector3& lensVertexPos, const Float time, UniPathState& pathState, SurfaceVertex& surfVertex, Vector3& bsdfContrib)
{
    const ShapeInst& shapeInst = surfVertex.shapeInst;
    const BSDF* bsdf = shapeInst.obj->bsdf.get();
    const Vector3& wi = pathState.wi;
    ...
    // do we change of parametrisation for high roughtness ?
    surfVertex.useAbsoluteParam = BoolToFloat(bsdf->Roughness(shapeInst.st, surfVertex.bsdfDiscrete) > roughnessThreshold);

    // todo @nico : ???
    if (surfVertex.useAbsoluteParam==FFALSE) {
        return false;
    }

    ray.dir = lensVertexPos-pathState.isect.position;
    const Float dist = Length(ray.dir);
    ray.dir *= inverse(dist);

    if (Occluded(scene, time, Ray{pathState.isect.position, ray.dir}, dist)) {
        return false;
    }
    surfVertex.bsdfRndParam = ToSphericalCoord(ray.dir);

    Float cosWo, bsdfPdfRev;
    bsdf->Evaluate(wi, isect.shadingNormal, ray.dir, shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfPdfRev);
    if (bsdfContrib.isZero() || bsdfPdf<=Float(0.0) || fabs(cosWo)<Float(1e-2) || fabs(Dot(ray.dir, isect.geomNormal))<Float(1e-2)) {
        // Be very strict on grazing angle: lens perturbation sometimes produces
        // vertices on the same surfaces
        return false;
    }
    ...
    bsdfContrib *= inverse(bsdfPdf);
    pathState.ssJacobian *= (bsdfPdf*fabs(sin(acos(ray.dir[2]))));
...
/*
 * perturb lens by calling PerturbLensFirstVertex
...
void PerturbLens(const Scene* scene, const Vector2 screenPos, Path& path, std::vector<SubpathContrib>& contribs)
{
    ...
    // set the lens pos that is the 2d pixel position in range [0..1][0..1]
    // this value has been modified by a lens mutation before
    camVertex.screenPos = screenPos;

    bool useLightCoordinatesPerturb = false;

    // this contain the current direction (pathState.raySeg), it will be updated at each iteration
    UniPathState pathState;
    Init(pathState);

    // Generate a new camera ray direction in pathState.raySeg using the new camVertex.screenPos and possible different time
    SamplePrimary(camera, camVertex.screenPos, path.time, pathState.raySeg);

    // loop over all sensor vertex path
    for (int camDepth = 0; camDepth<(int) path.camSurfaceVertex.size(); camDepth++) {
        ...
        // first vertex ? need to call Intersect first
        if (camDepth==0) {
            // first intersection
            hitSurface = Intersect(scene, path.time, pathState.raySeg, surfVertex.shapeInst, pathState.isect);
        } else {
            ...
            if (shapeInst.obj==nullptr) {
                ...
                Vector2 st;
                if (!shapeInst.obj->Intersect(shapeInst.primID, path.time, pathState.raySeg, pathState.isect, st)) {
                    return;
                }
            }
        }
        ...
        // Here we handle the case of being on the last vertex of the camera vertex list
        if (camDepth==(int) path.camSurfaceVertex.size()-1) {

            // is light vertex list empty (no surface vertex) ? (s=0, t>0) = pathtracing case
            if (path.lgtDepth==0) {

                // return emitter if we hit a light of env map
                const Light* light = GetHitLight(scene, hitSurface, surfVertex.shapeInst.obj);

                // but if we hit a light, we connect it and eval
                if (light!=nullptr) {
                    assert(camDepth>=2);
                    HandleHitLight(camDepth, scene, light, hitSurface, pathState, path.time, path.camVertex.screenPos, path.envLightInst, contribs);
                }

                //here path has zero contrib since it did not connect to any emitter (there is no envmap and did not hit a light)

            }
            else if (path.lgtDepth==1) {

                // can not connect to light path because last camera vertex hit nothing
                if (!hitSurface) {
                    return;
                }

                // todo @nico : this case is not clear
                if (camDepth==1) {
                    // todo @nico : is lensVertexPos a 3d pos on a lens ?
                    path.lensVertexPos = pathState.isect.position;
                    const Float distSq = DistanceSquared(pathState.isect.position, pathState.raySeg.ray.org);
                    if (distSq<=Float(0.0)) {
                        contribs.clear();
                ...
                // get prob of having sampled this light
                const Float directLightPickProb = PickLightProb(scene, surfVertex.directLightInst.light);

                // todo @nico: ???
                DirectLighting(camDepth, scene, pathState, path.time, path.camVertex.screenPos, directLightPickProb, false, surfVertex, contribs);
            }
            return;
        }

        // not hit ? no need to go further
        if (!hitSurface) {
            return;
        }
        ...
        // pertub first vertex if current vertex is the first vertex of the list
        if (camDepth==0) {
            // pertub vertex, if no pertub is done we exit because nothing change on the path
            if (!PerturbLensFirstVertex(scene, scene->options->roughnessThreshold, path.lensVertexPos, path.time, pathState, surfVertex, bsdfContrib)) {
                return;
            }
        } else {
            // here we are on a camera vertex that is not the first or last one
            if (useLightCoordinatesPerturb) {
                if (!LightCoordinateSampling(camDepth, scene, path.time, surfVertex, path.camSurfaceVertex.back(), false, pathState, bsdfContrib)) {
                    return;
                }
            } else {
                // because inboming dire wi has change we have to re-evaluate bsdf (path is changing)
                if (!BSDFSampling<true>(camDepth, scene->options->roughnessThreshold, pathState, surfVertex, bsdfContrib)) {
                    return;
                }
            }
...
bool PerturbLensLastVertex(const Scene* scene, const Camera* camera, const Float roughnessThreshold, const Vector2 screenPos, const Float time, BidirPathState& pathState, SurfaceVertex& curSurfVertex, SurfaceVertex& nextSurfVertex, Vector3& dir, Vector3& bsdfContrib)
{
    const BSDF* bsdf = curSurfVertex.shapeInst.obj->bsdf.get();
    curSurfVertex.useAbsoluteParam = BoolToFloat(bsdf->Roughness(curSurfVertex.shapeInst.st, curSurfVertex.bsdfDiscrete)>roughnessThreshold);
    if (curSurfVertex.useAbsoluteParam==FFALSE) {
        return false;
    }

    RaySegment raySeg;
    SamplePrimary(camera, screenPos, time, raySeg);
    Intersection nextIsect;
    if (!Intersect(scene, time, raySeg, nextSurfVertex.shapeInst, nextIsect)) {
        return false;
    }
    dir = Vector3(nextIsect.position-pathState.isect.position);
    const Float dist = Length(dir);
    dir *= inverse(dist);
    if (Occluded(scene, time, Ray{pathState.isect.position, dir}, dist)) {
        return false;
    }

    curSurfVertex.bsdfRndParam = ToSphericalCoord(dir);

    Float cosWo, bsdfPdf, bsdfRevPdf;
    bsdf->EvaluateAdjoint(pathState.wi, pathState.isect.shadingNormal, dir, curSurfVertex.shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    if (bsdfContrib.isZero() || bsdfPdf<=Float(0.0)) {
        return false;
    }
    const Float factor = ShadingNormalCorrection<true>(dir, pathState.isect, pathState.wi);
    ...
    bsdfContrib *= inverse(bsdfPdf);
    pathState.ssJacobian *= (bsdfPdf*fabs(sin(acos(dir[2]))));
    pathState.accMISWThis = MIS(cosWo/bsdfPdf)*(pathState.accMISWThis*MIS(bsdfRevPdf)+pathState.accMISWPrev);
    pathState.accMISWPrev = MIS(inverse(bsdfPdf));
    pathState.throughput = pathState.throughput.cwiseProduct(bsdfContrib);
...
bool PerturbLensFirstVertex(const Scene* scene, const Float roughnessThreshold, const Vector3& lensVertexPos, const Float time, BidirPathState& pathState, SurfaceVertex& surfVertex, Vector3& dir, Vector3& bsdfContrib)
{
    const ShapeInst& shapeInst = surfVertex.shapeInst;
    const BSDF* bsdf = shapeInst.obj->bsdf.get();
    const Vector3& wi = pathState.wi;
    ...
    dir = lensVertexPos-pathState.isect.position;
    const Float dist = Length(dir);
    dir *= inverse(dist);
    if (Occluded(scene, time, Ray{pathState.isect.position, dir}, dist)) {
        return false;
    }
    surfVertex.bsdfRndParam = ToSphericalCoord(dir);

    Float cosWo, bsdfPdf, bsdfRevPdf;
    bsdf->Evaluate(wi, isect.shadingNormal, dir, shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    if (bsdfContrib.isZero() || bsdfPdf<=Float(0.0) || fabs(cosWo)<Float(1e-2) || fabs(Dot(dir, isect.geomNormal))<Float(1e-2)) {
        // Be very strict on grazing angle: lens perturbation sometimes produces
        // vertices on the same surfaces
        return false;
    }
    pathState.lensContrib = pathState.lensContrib.cwiseProduct(bsdfContrib);
    bsdfContrib *= inverse(bsdfPdf);
    pathState.ssJacobian *= (bsdfPdf*fabs(sin(acos(dir[2]))));
    pathState.accMISWThis = MIS(cosWo/bsdfPdf)*(pathState.accMISWThis*MIS(bsdfRevPdf)+pathState.accMISWPrev);
    pathState.accMISWPrev = MIS(inverse(bsdfPdf));
...
void PerturbLensBidir(const Scene* scene, const Vector2 screenPos, Path& path, std::vector<SubpathContrib>& contribs)
{
    ...
    if (path.lgtDepth>1) {
        ...
        for (int lgtDepth = 0; lgtDepth<(int) path.lgtSurfaceVertex.size(); lgtDepth++) {
            SurfaceVertex& surfVertex = path.lgtSurfaceVertex[lgtDepth];
            ShapeInst& shapeInst = surfVertex.shapeInst;
            if (!shapeInst.obj->Intersect(shapeInst.primID, path.time, raySeg, lightPathState.isect, shapeInst.st)) {
                return;
            }
            ...
            Vector3 bsdfContrib;
            if (lgtDepth==(int) path.lgtSurfaceVertex.size()-2 && path.camDepth==1) {
                if (!PerturbLensLastVertex(scene, camera, scene->options->roughnessThreshold, screenPos, path.time, lightPathState, surfVertex, path.lgtSurfaceVertex.back(), raySeg.ray.dir, bsdfContrib)) {
                    return;
                }
            } else {
                if (!BSDFSampling<true, true>(scene->options->roughnessThreshold, lgtDepth, lightPathState, surfVertex, lightPathState, raySeg.ray.dir, bsdfContrib)) {
    ...
    for (int camDepth = 0; camDepth<(int) path.camSurfaceVertex.size(); camDepth++) {
        ...
        bool hitSurface = true;
        if (camDepth==0) {
            hitSurface = Intersect(scene, path.time, raySeg, surfVertex.shapeInst, camPathState.isect);
        } else {
            ShapeInst& shapeInst = surfVertex.shapeInst;
            if (shapeInst.obj==nullptr) {
                hitSurface = false;
                ...
                Vector2 st;
                if (!shapeInst.obj->Intersect(shapeInst.primID, path.time, raySeg, camPathState.isect, st)) {
                    return;
                }
            }
        }
        ...
        if (camDepth==(int) path.camSurfaceVertex.size()-1) {
            if (path.lgtDepth==1) {
                const Float directLightPickProb = PickLightProb(scene, surfVertex.directLightInst.light);
                assert(camDepth>=1);
                DirectLighting(camDepth, scene, path.time, camPathState, path.camVertex.screenPos, directLightPickProb, surfVertex, false, true, contribs);
            } else {  // path.lgtDepth > 1
                ConnectVertex(camDepth, path.lgtSurfaceVertex.size()-1, scene, path.time, lightPathState, path.lgtSurfaceVertex.back(), camPathState, surfVertex, path.camVertex.screenPos, camDepth==0, contribs);
            }
            return;
        }
        ...
        if (camDepth==0) {
            ...
            // todo @nico : what is this doing ?
            if (!PerturbLensFirstVertex(scene, scene->options->roughnessThreshold, path.lensVertexPos, path.time, camPathState, surfVertex, raySeg.ray.dir, bsdfContrib)) {
                // if no pertub done return
                return;
            }
        } else {
            if (useLightCoordinatesPerturb) {
                if (!LightCoordinateSampling(camDepth, scene, path.time, surfVertex, path.camSurfaceVertex.back(), false, camPathState, raySeg.ray.dir, bsdfContrib)) {
                    return;
                }
            } else {
                if (!BSDFSampling<false, true>(scene->options->roughnessThreshold, camDepth, camPathState, surfVertex, camPathState, raySeg.ray.dir, bsdfContrib)) {
...
static void HandleHitLight(const int camDepth, const ADBSphere& sceneSphere, const ADFloat* camBuffer, ADUniPathState& pathState, const ADFloat time, const ADVector2 screenPos)
{
    ...
    ADFloat directPdf;
    ADFloat emissionPdf;
    Emission(camBuffer, sceneSphere, ray.dir, isect.shadingNormal, time, emission, directPdf, emissionPdf);
...
static void DirectLighting(const int camDepth, const ADBSphere& sceneSphere, const ADFloat* camBuffer, ADUniPathState& pathState, const ADVector2 rndParam, const ADFloat time, const ADVector2 screenPos, const bool isStatic)
{
    const ADIntersection& isect = pathState.isect;
    ADVector3& throughput = pathState.throughput;
    ...
    camBuffer = SampleDirect(camBuffer, sceneSphere, isect.position, isect.shadingNormal, rndParam, time, isStatic, dirToLight, lightContrib, cosAtLight, directPdf, emissionPdf);
    ...
    camBuffer = EvaluateBSDF(false, camBuffer, wi, isect.shadingNormal, dirToLight, pathState.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    ...
    if (camDepth==0) {
        ...
        EvaluateBSDF(true, bsdfBuffer, dirToLight, isect.shadingNormal, wi, pathState.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfRevPdf);
...
static const ADFloat* BSDFSampling(const int camDepth, const ADFloat* buffer, const ADVector2 bsdfRndParam, const ADFloat bsdfDiscrete, const ADFloat useAbsoluteParam, const ADFloat time, const bool isStatic, const ADFloat useLightCoordinateSampling, const bool doLightCoordinateSampling, ADUniPathState& pathState)
{
    const ADVector3& wi = pathState.wi;
    const ADIntersection& isect = pathState.isect;
    ...
    if (doLightCoordinateSampling) {
        ...
        BeginIf(And(Eq(useLightCoordinateSampling, FTRUE), Eq(lightType, Float(LightType::AreaLight))), ret);
        {
            ADVector3 dir;
            ADVector3 bsdfContrib;
            ADFloat cosWo;
            ...
            dir *= invDist;
            EvaluateBSDF(false, buffer, wi, isect.shadingNormal, dir, pathState.st, bsdfContrib, cosWo, bsdfPdf, bsdfPdfRev);
            ADFloat jacobian = fabs(Dot(nextNormal, dir)*invDistSq)/shapePdf;
            SetCondOutput({dir[0], dir[1], dir[2], bsdfContrib[0], bsdfContrib[1], bsdfContrib[2], bsdfPdf, jacobian});
        }
    ...
    }
    {
        ...
        ADFloat bsdfPdf;
        ADFloat bsdfPdfRev;
        SampleBSDF(false, buffer, wi, isect.shadingNormal, pathState.st, bsdfRndParam, bsdfDiscrete, fixedDiscrete, dir, bsdfContrib, cosWo, bsdfPdf, bsdfPdfRev);
        SetCondOutput({dir[0], dir[1], dir[2], bsdfContrib[0], bsdfContrib[1], bsdfContrib[2], bsdfPdf, Const<ADFloat>(1.0)});
    }
    BeginElse();
    {
        ADVector3 dir;
    ...
    if (camDepth==0) {
        ...
        EvaluateBSDF(true, bsdfBuffer, wi, isect.shadingNormal, ray.dir, pathState.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfPdfRev);
...
static const ADFloat* PerturbLensFirstVertex(const ADFloat* buffer, const ADVector3& lensVertexPos, ADUniPathState& pathState)
{
    const ADVector3& wi = pathState.wi;
    const ADIntersection& isect = pathState.isect;
    ADRay& ray = pathState.ray;

    ray.dir = Normalize(ADVector3(lensVertexPos-pathState.isect.position));
    ADVector3 bsdfContrib;
    ADFloat cosWo;
    ADFloat bsdfPdfRev;
    ADFloat jacobian;
    buffer = EvaluateBSDF(false, buffer, wi, isect.shadingNormal, ray.dir, pathState.st, bsdfContrib, cosWo, pathState.lastBsdfPdf, bsdfPdfRev);
    ...
    for (int camDepth = 0; camDepth<(int) path.camSurfaceVertex.size(); camDepth++) {
        ...
        if (camDepth==(int) path.camSurfaceVertex.size()-1) {
            if (path.lgtDepth==0) {
                ...
                surfVertex.directLightInst.light->Serialize(surfVertex.directLightInst.lPrimID, buffer);
                buffer += GetMaxLightSerializedSize();
                shapeInst.obj->bsdf->Serialize(shapeInst.st, buffer);
                buffer += GetMaxBSDFSerializedSize();
...
std::string RegisterPathFunc(const int maxCamDepth,
    ...
    std::cout << "Generating function " << funcName << std::endl;
    auto func = BeginFunction(funcName, std::vector<Argument>{lensParamsArg, primaryParamsArg, sceneParamsArg, vertParamsArg});
    ...
    pathState.lensThroughput = ADVector3(Const<ADFloat>(1.0), Const<ADFloat>(1.0), Const<ADFloat>(1.0));
    ...
    for (int camDepth = 0; camDepth<maxCamDepth-1; camDepth++) {
        ...
        if (camDepth==maxCamDepth-2) {
            if (maxLightDepth==0) {
                ...
                DirectLighting(camDepth, sceneSphere, buffer, pathState, directLightRndParam, time, screenPos, isStatic);
            }
            break;
        }

        if (camDepth==1) {
            pathState.lensThroughput *= inverse(DistanceSquared(pathState.isect.position, pathState.ray.org));
        }
        ...
        if (mode==PathFuncMode::Lens && camDepth==0) {
            ...
            buffer = BSDFSampling<false>(camDepth, buffer, bsdfRndParam, bsdfDiscrete, useAbsoluteParam, time, isStatic, useLightCoordinateSampling, doLightCoordinateSampling, pathState);
        }
        ADFloat rrWeight;
    ...
    ADFloat logLumValue = log(Luminance(mode==PathFuncMode::Lens ? pathState.lensThroughput : pathState.throughput));
    ...
    lib.RegisterFuncDerv(func, wrt, logLumValue, false, hessian);
    return func->name;
}
...
static const ADFloat* EmitFromLight(const ADFloat* buffer, const ADBSphere& bSphere, const ADFloat lightPickProb, const ADFloat time, const bool isStatic, const ADVector2 rndParamPos, const ADVector2 rndParamDir, ADRay& ray,ADBidirPathState& pathState)
{
    ADFloat lightType;
    ...
    ADFloat emissionPdf;
    ADFloat directPdf;
    buffer = Emit(buffer, bSphere, rndParamPos, rndParamDir, time, isStatic, ray, pathState.throughput, cosLight, emissionPdf, directPdf);
    emissionPdf *= lightPickProb;
    directPdf *= lightPickProb;
    pathState.throughput *= inverse(lightPickProb);
    pathState.accMISWPrev = MIS(directPdf/emissionPdf);
    ...
    pathState.lensContrib = ADVector3(Const<ADFloat>(0.0), Const<ADFloat>(0.0), Const<ADFloat>(0.0));
    return buffer;
}

static inline void ConvertMISLightEmit(const ADFloat lightType, const ADRay& ray, ADBidirPathState& pathState)
{
    std::vector<CondExprCPtr> ret = CreateCondExprVec(1);
...
static inline void ConvertMISLightHit(const ADFloat lightType, const ADRay& ray, ADBidirPathState& pathState)
{
    std::vector<CondExprCPtr> ret = CreateCondExprVec(2);
    BeginIf(Eq(lightType, Float(LightType::EnvLight)), ret);
    { SetCondOutput({Const<ADFloat>(1.0), Const<ADFloat>(1.0)}); }
...
static void ConnectToCamera(const ADMatrix4x4& sampleToCam, const ADAnimatedTransform& camToWorld, const ADFloat screenPixelCount, const ADFloat camDist, const ADFloat* buffer, const ADFloat time, const bool isStatic, const ADVector3& prevLensContrib, const ADVector3& prevPosition, ADBidirPathState& pathState)
{
    ADRay centerRay;
    SamplePrimary(sampleToCam, camToWorld, ADVector2(Const<ADFloat>(0.5), Const<ADFloat>(0.5)), time, isStatic, centerRay);
    ...
    const ADFloat* bsdfBuffer = buffer;
    buffer = EvaluateBSDF(true, bsdfBuffer, pathState.wi, pathState.isect.shadingNormal, dirToCamera, pathState.st, bsdfContrib, cosToCamera, bsdfPdf, bsdfRevPdf);
...

    {
        ...
        EvaluateBSDF(false, bsdfBuffer, dirToCamera, pathState.isect.shadingNormal, pathState.wi, pathState.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);

        pathState.lensContrib = bsdfContrib.cwiseProduct(prevLensContrib)*inverse(DistanceSquared(pathState.isect.position, prevPosition));
    }
    ...
    ADFloat wLight = MIS(cameraPdf/screenPixelCount)*(pathState.accMISWPrev+pathState.accMISWThis*MIS(bsdfRevPdf));
...
static const ADFloat* BSDFSampling(const int depth, const ADFloat* buffer, const ADVector2 bsdfRndParam, const ADFloat bsdfDiscrete, const ADFloat useAbsoluteParam, const ADFloat time, const bool isStatic, const ADFloat useLightCoordinateSampling, const bool doLightCoordinateSampling, ADBidirPathState& pathState, ADVector3& dir)
{
    const ADIntersection& isect = pathState.isect;
    ADVector3 bsdfContrib;
    ADFloat cosWo, bsdfPdf, bsdfRevPdf;
    ...
    if (doLightCoordinateSampling) {
        ...
        BeginIf(And(Eq(useLightCoordinateSampling, FTRUE), Eq(lightType, Float(LightType::AreaLight))), ret);
        {
            ...
            ADVector3 nextPosition, nextNormal;
            ADFloat shapePdf;
            SampleShape(nextShapeBuffer, bsdfRndParam, time, isStatic, nextPosition, nextNormal, shapePdf);
            dir = nextPosition-isect.position;
            ADFloat distSq = LengthSquared(dir);
            ADFloat invDistSq = inverse(distSq);
            ADFloat invDist = sqrt(invDistSq);
            dir *= invDist;
            EvaluateBSDF(false, buffer, pathState.wi, isect.shadingNormal, dir, pathState.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
            jacobian = fabs(Dot(nextNormal, dir)*invDistSq)/shapePdf;
            SetCondOutput({dir[0], dir[1], dir[2], bsdfContrib[0], bsdfContrib[1], bsdfContrib[2], cosWo, bsdfPdf, bsdfRevPdf, jacobian});
        }
        BeginElseIf(Eq(useAbsoluteParam, FFALSE));
    } else {
        BeginIf(Eq(useAbsoluteParam, FFALSE), ret);
    }
    {
        ...
        ADFloat bsdfPdf;
        ADFloat bsdfRevPdf;
        SampleBSDF(adjoint, buffer, pathState.wi, isect.shadingNormal, pathState.st, bsdfRndParam, bsdfDiscrete, fixedDiscrete, dir, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
        SetCondOutput({dir[0], dir[1], dir[2], bsdfContrib[0], bsdfContrib[1], bsdfContrib[2], cosWo, bsdfPdf, bsdfRevPdf, Const<ADFloat>(1.0)});
    }
    BeginElse();
    {
        ADVector3 dir;
        ...
        ADFloat jacobian;
        dir = SampleSphere(bsdfRndParam, jacobian);
        EvaluateBSDF(adjoint, buffer, pathState.wi, isect.shadingNormal, dir, pathState.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
        SetCondOutput({dir[0], dir[1], dir[2], bsdfContrib[0], bsdfContrib[1], bsdfContrib[2], cosWo, bsdfPdf, bsdfRevPdf, jacobian});
    }
    EndIf();
    buffer += GetMaxBSDFSerializedSize();
    ...
    if (adjoint) {
        ...
        ADFloat impBsdfPdf;
        ADFloat impBsdfPdfRev;
        EvaluateBSDF(true, bsdfBuffer, dir, isect.shadingNormal, pathState.wi, pathState.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfPdfRev);
...
static void EmitFromCamera(const ADFloat time, const bool isStatic, const ADMatrix4x4& sampleToCam, const ADAnimatedTransform& camToWorld, const ADFloat screenPixelCount, const ADFloat camDist, const ADVector2 screenPos, ADRay& ray, ADBidirPathState& pathState)
{
    ADRay centerRay;
    SamplePrimary(sampleToCam, camToWorld, ADVector2(Const<ADFloat>(0.5), Const<ADFloat>(0.5)), time, isStatic, centerRay);
    ...
    pathState.lensContrib = ADVector3(Const<ADFloat>(1.0), Const<ADFloat>(1.0), Const<ADFloat>(1.0));
}

static void HandleHitLight(const ADBSphere& sceneSphere, const ADFloat* buffer, const ADVector3& dir, const ADFloat time, ADBidirPathState& pathState)
{
    ADVector3 emission;
    ADFloat directPdf;
    ADFloat emissionPdf;
    buffer = Emission(buffer, sceneSphere, dir, pathState.isect.shadingNormal, time, emission, directPdf, emissionPdf);
    ...
    // accMISWPrev magically accounts for measure conversion
    ADFloat wCamera = MIS(directPdf)*pathState.accMISWPrev+MIS(emissionPdf)*pathState.accMISWThis;
    ADFloat misWeight = inverse(Float(1.0)+wCamera);
    pathState.throughput *= misWeight;
}

static void DirectLighting(const int camDepth, const ADBSphere& sceneSphere, const ADFloat* buffer, const ADFloat time, const bool isStatic, ADBidirPathState& pathState, const ADVector2 directLightRndParam)
{
    ADVector3 dirToLight;
    ADVector3 lightContrib;
    ADFloat cosAtLight;
    ...
    ADFloat lightType;
    Deserialize(buffer, lightType);
    buffer = SampleDirect(buffer, sceneSphere, pathState.isect.position, pathState.isect.shadingNormal, directLightRndParam, time, isStatic, dirToLight, lightContrib, cosAtLight, directPdf, emissionPdf);
    ...
    ADFloat bsdfRevPdf;
    const ADFloat* bsdfBuffer = buffer;
    buffer = EvaluateBSDF(false, bsdfBuffer, pathState.wi, pathState.isect.shadingNormal, dirToLight, pathState.st, bsdfContrib, cosToLight, bsdfPdf, bsdfRevPdf);
    ...
    if (camDepth==1) {
        ...
        ADVector3 impBsdfContrib;
        ADFloat impCosWo, impBsdfPdf, impBsdfRevPdf;
        EvaluateBSDF(true, bsdfBuffer, dirToLight, pathState.isect.shadingNormal, pathState.wi, pathState.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfRevPdf);
        // Adjoint BSDF correction
    ...
    ADFloat wCamera = MIS(emissionPdf*cosToLight/(directPdf*cosAtLight))*(pathState.accMISWPrev+pathState.accMISWThis*MIS(bsdfRevPdf));
    ADFloat misWeight = inverse(wLight+Float(1.0)+wCamera);
    pathState.throughput *= misWeight;
}

static void ConnectVertex(const int camDepth, const ADFloat* buffer, const ADFloat* lgtBSDFBuffer, const ADBidirPathState& lgtPathState, ADBidirPathState& camPathState)
{
    ...
    buffer = EvaluateBSDF(false, camBsdfBuffer, camPathState.wi, camPathState.isect.shadingNormal, dirToLight, camPathState.st, camBsdfFactor, cosCamera, camBsdfPdf, camBsdfRevPdf);
    ...
    EvaluateBSDF(true, lgtBSDFBuffer, lgtPathState.wi, lgtPathState.isect.shadingNormal, -dirToLight, lgtPathState.st, lgtBsdfFactor, cosLight, lgtBsdfPdf, lgtBsdfRevPdf);

    ADFloat lgtFactor = ShadingNormalCorrection<true>(lgtPathState.wi, lgtPathState.isect, -dirToLight);
    lgtBsdfFactor *= lgtFactor;
    ...
    if (camDepth==0) {
        ...
        ADVector3 impBsdfContrib;
        ADFloat impCosWo, impBsdfPdf, impBsdfRevPdf;
        EvaluateBSDF(true, camBsdfBuffer, dirToLight, isect.shadingNormal, camPathState.wi, camPathState.st, impBsdfContrib, impCosWo, impBsdfPdf, impBsdfRevPdf);
        // Adjoint BSDF correction
    ...
    ADFloat wLight = MIS(camBsdfDirPdfA)*(lgtPathState.accMISWPrev+lgtPathState.accMISWThis*MIS(lgtBsdfRevPdf));
    ADFloat wCamera = MIS(lgtBsdfDirPdfA)*(camPathState.accMISWPrev+camPathState.accMISWThis*MIS(camBsdfRevPdf));
    ADFloat misWeight = inverse(wLight+Float(1.0)+wCamera);
    ...
    camPathState.throughput = camPathState.throughput.cwiseProduct(lgtBsdfFactor)*(geometryTerm*misWeight);
}

static const ADFloat* PerturbLensLastVertex(const ADFloat* buffer, const ADMatrix4x4& sampleToCam, const ADAnimatedTransform& camToWorld, const ADVector2 screenPos, const ADFloat time, const bool isStatic, const ADFloat useAbsoluteParam, ADBidirPathState& pathState, ADVector3& dir)
{
    ADRay ray;
    SamplePrimary(sampleToCam, camToWorld, screenPos, time, isStatic, ray);
    // remember to take account for rrWeight
    const ADFloat* nextShapeBuffer = buffer+GetMaxBSDFSerializedSize()+1;
    ADIntersection nextIsect;
    ADVector2 st;
    Intersect(nextShapeBuffer, ray, time, isStatic, nextIsect, st);
    dir = Normalize(ADVector3(nextIsect.position-pathState.isect.position));

    ADVector3 bsdfContrib;
    ADFloat cosWo;
    ADFloat bsdfPdf, bsdfRevPdf;
    buffer = EvaluateBSDF(true, buffer, pathState.wi, pathState.isect.shadingNormal, dir, pathState.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    ADFloat factor = ShadingNormalCorrection<true>(dir, pathState.isect, pathState.wi);
    bsdfContrib *= factor;
    pathState.lensContrib = bsdfContrib;
    return buffer;
}

static const ADFloat* PerturbLensFirstVertex(const ADFloat* buffer, const ADVector3& lensVertexPos, ADBidirPathState& pathState, ADVector3& dir)
{
    dir = Normalize(ADVector3(lensVertexPos-pathState.isect.position));
    ADVector3 bsdfContrib;
    ADFloat cosWo, bsdfPdf, bsdfRevPdf;
    buffer = EvaluateBSDF(false, buffer, pathState.wi, pathState.isect.shadingNormal, dir, pathState.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    pathState.lensContrib = pathState.lensContrib.cwiseProduct(bsdfContrib);
    return buffer;
}
...
std::string RegisterPathFuncBidir(const int maxCamDepth,
    ...
    std::cout << "Generating function " << funcName << std::endl;
    auto func = BeginFunction(funcName, {lensParamsArg, primaryParamsArg, sceneParamsArg, vertParamsArg});
    auto lensParams = lensParamsArg.GetExprVec();
    auto primaryParams = primaryParamsArg.GetExprVec();
    auto sceneParams = sceneParamsArg.GetExprVec();
    auto vertParams = vertParamsArg.GetExprVec();
    ...
    if (maxLightDepth>1) {
        ...
        ADFloat lightType;
        Deserialize(buffer, lightType);
        buffer = EmitFromLight(buffer, sceneSphere, lightPickProb, time, isStatic, rndParamPos, rndParamDir, ray, lightPathState);
        ...
        for (int lgtDepth = 0; lgtDepth<maxLightDepth-1; lgtDepth++) {
            buffer = Intersect(buffer, ray, time, isStatic, lightPathState.isect, lightPathState.st);
            ...
            if (lgtDepth==maxLightDepth-2) {
                if (maxCamDepth==1) {
                    ConnectToCamera(sampleToCam, camToWorld, screenPixelCount, camDist, buffer, time, isStatic, prevLensContrib, ray.org, lightPathState);
                    if (mode==PathFuncMode::Lens) {
            ...
            if (mode==PathFuncMode::Lens && lgtDepth==maxLightDepth-3 && maxCamDepth==1) {
                buffer = PerturbLensLastVertex(buffer, sampleToCam, camToWorld, screenPos, time, isStatic, useAbsoluteParam, lightPathState, ray.dir);
            } else {
                buffer = BSDFSampling<true, false>(lgtDepth, buffer, bsdfRndParam, bsdfDiscrete, useAbsoluteParam, time, isStatic, useLightCoordinateSampling, false, lightPathState, ray.dir);
            }
            prevLensContrib = lightPathState.lensContrib;
            ADFloat rrWeight;
            buffer = Deserialize(buffer, rrWeight);
    ...
    if (maxCamDepth>1) {
        ...
        EmitFromCamera(time, isStatic, sampleToCam, camToWorld, screenPixelCount, camDist, screenPos, ray, camPathState);
        ...
        for (int camDepth = 0; camDepth<maxCamDepth-1; camDepth++) {
            ...
            if (mode==PathFuncMode::Lens && camDepth==1) {
                camPathState.lensContrib *= inverse(DistanceSquared(camPathState.isect.position, ray.org));
            }
            ...
            if (camDepth==maxCamDepth-2) {
                if (maxLightDepth==1) {
                    ...
                    directLightRndParam[0] = primaryParams[primaryIdx++];
                    directLightRndParam[1] = primaryParams[primaryIdx++];
                    DirectLighting(camDepth, sceneSphere, buffer, time, isStatic, camPathState, directLightRndParam);
                } else {  // maxLgtDepth > 1
            ...
            if (mode==PathFuncMode::Lens && camDepth==0) {
                ...
                bool doLightCoordinateSampling = maxLightDepth==0 && camDepth==maxCamDepth-3;
                buffer = BSDFSampling<false, false>(camDepth, buffer, bsdfRndParam, bsdfDiscrete, useAbsoluteParam, time, isStatic, useLightCoordinateSampling, doLightCoordinateSampling, camPathState, ray.dir);
            }
            ADFloat rrWeight;
            buffer = Deserialize(buffer, rrWeight);
...
std::shared_ptr<Library> CompilePathFuncLibrary(const bool bidirectional,
    ...
    for (int maxCamDepth = 1; maxCamDepth<=maxDepth+1; maxCamDepth++) {
        for (int maxLgtDepth = 0; maxLgtDepth<=(bidirectional ? maxDepth : 1); maxLgtDepth++) {
            ...
            if (bidirectional) {
                RegisterPathFuncBidir(maxCamDepth, maxLgtDepth, PathFuncMode::Full, *pathLib, hessian);
                RegisterPathFuncBidir(maxCamDepth, maxLgtDepth, PathFuncMode::Static, *pathLib, hessian);
                if (maxCamDepth + maxLgtDepth > 3) {
                    RegisterPathFuncBidir(maxCamDepth, maxLgtDepth, PathFuncMode::Lens, *pathLib, hessian);
                }
            } else {
                RegisterPathFunc(maxCamDepth, maxLgtDepth, PathFuncMode::Full, *pathLib, hessian);
                RegisterPathFunc(maxCamDepth, maxLgtDepth, PathFuncMode::Static, *pathLib, hessian);
                if (maxCamDepth + maxLgtDepth > 3) {
                    RegisterPathFunc(maxCamDepth, maxLgtDepth, PathFuncMode::Lens, *pathLib, hessian);
                }
            }
        }
    }
...
std::shared_ptr<const PathFuncLib> BuildPathFuncLibrary(const bool bidirectional, const int maxDepth, bool hessian) {
    std::shared_ptr<Library> pathLib = std::make_shared<Library>(GetLibPath(), bidirectional ? "pathlibbidir" : "pathlib");
    if (!pathLib->IsLinked())
        pathLib = CompilePathFuncLibrary(bidirectional, maxDepth, hessian, &pathLib);
    ...
    for (int maxCamDepth = 1; maxCamDepth<=maxDepth+1; maxCamDepth++) {
        for (int maxLgtDepth = 0; maxLgtDepth<=(bidirectional ? maxDepth : 1); maxLgtDepth++) {
            ...
            std::string funcName = bidirectional ? GetFuncNameBidir(maxCamDepth, maxLgtDepth, PathFuncMode::Full) : GetFuncName(maxCamDepth, maxLgtDepth, PathFuncMode::Full);
            {
                PathFunc f = (PathFunc) pathLib->GetFunc(funcName);
                if (f!=nullptr) {
                    funcMap[{maxCamDepth, maxLgtDepth}] = f;
                } else {
                    std::cout << "[Warning] Can't find function:" << funcName << std::endl;
                }
            }
            {
                PathFuncDerv f = (PathFuncDerv) pathLib->GetFuncDerv(funcName);
                if (f!=nullptr) {
                    funcDervMap[{maxCamDepth, maxLgtDepth}] = f;
                } else {
                    std::cout << "[Warning] Can't find derivatives function:" << funcName << std::endl;
                }
            }

            std::string staticFuncName = bidirectional ? GetFuncNameBidir(maxCamDepth, maxLgtDepth, PathFuncMode::Static) : GetFuncName(maxCamDepth, maxLgtDepth, PathFuncMode::Static);
            {
            ...
            std::string lensFuncName = bidirectional ? GetFuncNameBidir(maxCamDepth, maxLgtDepth, PathFuncMode::Lens) : GetFuncName(maxCamDepth, maxLgtDepth, PathFuncMode::Lens);
            if (maxCamDepth+maxLgtDepth>3) {
                {
                    PathFunc f = (PathFunc) pathLib->GetFunc(lensFuncName);
                    if (f!=nullptr) {
                        lensFuncMap[{maxCamDepth, maxLgtDepth}] = f;
                    } else {
                        std::cout << "[Warning] Can't find function:" << funcName << std::endl;
                    }
                }
                {
                    PathFuncDerv f = (PathFuncDerv) pathLib->GetFuncDerv(lensFuncName);
                    if (f!=nullptr) {
                        lensFuncDervMap[{maxCamDepth, maxLgtDepth}] = f;
                    } else {
                        std::cout << "[Warning] Can't find derivatives function:" << funcName << std::endl;
                    }
                }
    ...
    return std::make_shared<PathFuncLib>(PathFuncLib{maxDepth, pathLib, funcMap, funcDervMap, staticFuncMap, staticFuncDervMap, lensFuncMap, lensFuncDervMap});
}