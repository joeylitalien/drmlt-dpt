...
std::unique_ptr<Scene> ParseSceneNode(pugi::xml_node node, const fs::path& path);
std::shared_ptr<const Camera> ParseSensorNode(pugi::xml_node node, std::string &filename, const fs::path &path);
std::shared_ptr<Image3> ParseFilmNode(pugi::xml_node node, std::string &filename, const fs::path &path);
std::shared_ptr<const Shape> ParseShapeNode(pugi::xml_node node, const BSDFMap &bsdfMap, const TextureMap &textureMap, std::shared_ptr<const Light> &areaLight, const fs::path &path);
std::shared_ptr<const BSDF> ParseBSDFNode(pugi::xml_node node, const TextureMap &textureMap, const fs::path &path, bool twoSided = false);
std::shared_ptr<const Light> ParseEmitterNode(pugi::xml_node node, std::shared_ptr<const EnvLight> &envLight, const fs::path &path);
std::shared_ptr<const TextureRGB> ParseTextureNode(pugi::xml_node node, const fs::path &path);
std::shared_ptr<const Texture1D> Parse1DMapNode(pugi::xml_node node, const TextureMap &textureMap, const fs::path &path);
std::shared_ptr<const TextureRGB> Parse3DMapNode(pugi::xml_node node, const TextureMap &textureMap, const fs::path &path);
...
std::shared_ptr<Image3> ParseFilmNode(pugi::xml_node node, std::string &filename, const fs::path &path) {
    ...
    for (auto child : node.children()) {
        ...
        if (name == "width") {
            ...
            filename = path / std::string(child.attribute("value").value());
        }
    }
...
std::shared_ptr<const Camera> ParseSensorNode(pugi::xml_node node, std::string &filename, const fs::path &path) {
    ...
    for (auto child : node.children()) {
        ...
        if (name == "nearClip") {
            ...
            film = ParseFilmNode(child, filename, path);
        }
    }
    if (film.get() == nullptr) {
    ...
    return std::allocate_shared<Camera>(Eigen::aligned_allocator<Camera>(), toWorld, fov, film, nearClip, farClip);
}
...
std::shared_ptr<const Shape> ParseShapeNode(pugi::xml_node node, const BSDFMap &bsdfMap, const TextureMap &textureMap, std::shared_ptr<const Light> &areaLight, const fs::path &path) {
    ...
    for (auto child : node.children()) {
        std::string name = child.name();
        if (name == "bsdf") {
            bsdf = ParseBSDFNode(child, textureMap, path);
            break;
        } else if (name == "ref") {
    ...
    if (type == "serialized") {
        ...
        shape = std::make_shared<TriangleMesh>(bsdf, LoadSerialized(path / filename, shapeIndex, toWorld[0], toWorld[1], isMoving));
    } else if (type == "obj") {
        std::string filename;
        Matrix4x4 toWorld[2];
        toWorld[0] = toWorld[1] = Matrix4x4::Identity();
        ...
        shape = std::make_shared<TriangleMesh>(bsdf, ParseObj(path / filename, toWorld[0], toWorld[1], isMoving));
    }
...
std::shared_ptr<const BSDF> ParseBSDFNode(pugi::xml_node node,
                                          const TextureMap &textureMap, const fs::path &path,
                                          bool twoSided) {
    std::string type = node.attribute("type").value();
    if (type == "diffuse") {
        ...
        for (auto child : node.children()) {
            std::string name = child.attribute("name").value();
            if (name == "reflectance") {
                reflectance = Parse3DMapNode(child, textureMap, path);
            }
        }
        ...
        for (auto child : node.children()) {
            ...
            if (name == "diffuseReflectance") {
                diffuseReflectance = Parse3DMapNode(child, textureMap, path);
            } else if (name == "specularReflectance") {
                std::string value = child.attribute("value").value();
                specularReflectance = Parse3DMapNode(child, textureMap, path);
            } else if (name == "exponent") {
                exponent = Parse1DMapNode(child, textureMap, path);
            }
        }
        ...
        for (auto child : node.children()) {
            ...
            if (name == "intIOR") {
                ...
                alpha = Parse1DMapNode(child, textureMap, path);
            } else if (name == "specularReflectance") {
                std::string value = child.attribute("value").value();
                specularReflectance = Parse3DMapNode(child, textureMap, path);
            } else if (name == "specularTransmittance") {
                std::string value = child.attribute("value").value();
                specularTransmittance = Parse3DMapNode(child, textureMap, path);
            }
        }
        ...
        for (auto child : node.children()) {
            if (std::string(child.name()) == "bsdf") {
                return ParseBSDFNode(child, textureMap, path, true);
            }
        }
    }
    Error("Unknown BSDF");
    ...
    if (type == "bitmap") {
        ...
        for (auto child : node.children()) {
            ...
            if (name == "filename") {
                ...
                sScale = std::stof(child.attribute("value").value());
            } else if (name == "vscale") {
                tScale = std::stof(child.attribute("value").value());
            } else if (name == "uvscale") {
                sScale = tScale = std::stof(child.attribute("value").value());
            }
        }
        return std::make_shared<BitmapTextureRGB>(path / filename, Vector2(sScale, tScale));
    }
...
template <int N>
std::shared_ptr<const Texture<N>> ParseNDMap(pugi::xml_node node, const TextureMap &textureMap, const fs::path &path) {
    std::shared_ptr<const TextureRGB> reflectance;
    std::string nodeType = node.name();
    if (nodeType == "texture") {
        reflectance = ParseTextureNode(node, path);
...
std::shared_ptr<const TextureRGB> Parse3DMapNode(pugi::xml_node node, const TextureMap &textureMap, const fs::path &path) {
    return ParseNDMap<3>(node, textureMap, path);
}

std::shared_ptr<const Texture1D> Parse1DMapNode(pugi::xml_node node, const TextureMap &textureMap, const fs::path &path) {
    return ParseNDMap<1>(node, textureMap, path);
}
...
std::shared_ptr<const Light> ParseEmitterNode(pugi::xml_node node, std::shared_ptr<const EnvLight> &envLight, const fs::path &path) {
    ...
    if (type == "point") {
        ...
        for (auto child : node.children()) {
            ...
            if (name == "filename") {
                filename = child.attribute("value").value();
            } else if (name == "samplingWeight") {
                samplingWeight = std::stof(child.attribute("value").value());
            } else if (name == "toWorld") {
                if (std::string(child.name()) == "transform") {
                    Matrix4x4 m = ParseTransform(child);
                    toWorld = MakeAnimatedTransform(m, m);
        ...
        envLight = std::make_shared<EnvLight>(samplingWeight, toWorld, path / filename);
        return envLight;
    }
...
std::shared_ptr<DptOptions> ParseDptOptions(pugi::xml_node node) {
    ...
    for (auto child : node.children()) {
        ...
        } else if (name == "uniformmixingprobability") {
        ...
        } else if (name == "reportintervalseconds") {
            dptOptions->reportIntervalSeconds = std::stoi(child.attribute("value").value());
        } else if (name == "uselightcoordinatesampling") {
            dptOptions->useLightCoordinateSampling = child.attribute("value").value() == std::string("true");
        } else if (name == "largestepmultiplexed") {
            dptOptions->largeStepMultiplexed =
                child.attribute("value").value() == std::string("true");
        } 
        // Delayed rejection specifics
        else if (name == "type") {
            dptOptions->type = child.attribute("value").value();
        } else if (name == "anisoperturbstddev") {
            dptOptions->lensPerturbStdDev = std::stof(child.attribute("value").value());
        } else if (name == "anisoperturbprob") {
            dptOptions->anisoPerturbProb = std::stof(child.attribute("value").value());
        } else if (name == "acceptanceMap") {
            dptOptions->acceptanceMap = child.attribute("value").value() == std::string("true");
        } else {
...
std::unique_ptr<Scene> ParseSceneNode(pugi::xml_node node, const fs::path& path) {
    ...
    std::string outputName = path / "image.exr";
    for (auto child : node.children()) {
        std::string name = child.name();
        if (name == "sensor") {
            camera = ParseSensorNode(child, outputName, path);
        } else if (name == "shape") {
            std::shared_ptr<const Light> areaLight;
            objs.push_back(ParseShapeNode(child, bsdfMap, textureMap, areaLight, path));
            if (areaLight.get() != nullptr) {
                lights.push_back(areaLight);
            }
        } else if (name == "bsdf") {
            std::string id = child.attribute("id").value();
            bsdfMap[id] = ParseBSDFNode(child, textureMap, path);
        } else if (name == "emitter") {
            lights.push_back(ParseEmitterNode(child, envLight, path));
        } else if (name == "texture") {
            std::string id = child.attribute("id").value();
            textureMap[id] = ParseTextureNode(child, path);
        } else if (name == "dpt") {
...
std::unique_ptr<Scene> ParseScene(const std::string &filename) {
    ...
    if (result) {
        scene = ParseSceneNode(doc.child("scene"), fs::path(filename).parent_path());
    } else {
